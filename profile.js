const loadUserAccount = () => {
    ...
    fetch(`https://picku-pet-website-deployment1.vercel.app/user/UserProfileDetail/${fetchId}/`)
        .then(response => {
            if (!response.ok) {
...
async function fetchAndDisplayPets() {
    try {
        ...
        const userId = localStorage.getItem("user_id");
        if (!fetchId || fetchId === userId) {
            const adoptedPetsAPI = `https://picku-pet-website-deployment1.vercel.app/pets/petlist/?adopted_by=${userId}`;
            const adoptedResponse = await fetch(adoptedPetsAPI);
            const adoptedData = await adoptedResponse.json();
            
            const sellingRescuedPetsAPI = `https://picku-pet-website-deployment1.vercel.app/pets/petlist/?created_by=${userId}`;
            ...
            const adoptedPetsAPI = `https://picku-pet-website-deployment1.vercel.app/pets/petlist/?adopted_by=${fetchId}`;
            const adoptedResponse = await fetch(adoptedPetsAPI);
            const adoptedData = await adoptedResponse.json();
            
            const sellingRescuedPetsAPI = `https://picku-pet-website-deployment1.vercel.app/pets/petlist/?created_by=${fetchId}`;
            const sellingResponse = await fetch(sellingRescuedPetsAPI);
            const sellingData = await sellingResponse.json();
...
async function fetchData(key) {
    const url = `https://picku-pet-website-deployment1.vercel.app/pets/${key}/`;
